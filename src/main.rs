...
fn main() {
    let bitmap = lodepng::decode24_file("0.png").unwrap();
    let (width, height) = (bitmap.width, bitmap.height);
    println!("width={},height={}", width, height);

    let buffer = edge_detection(&bitmap.buffer, width, height, 127.0);
    
    lodepng::encode24_file("0_out.png", &buffer, width, height).unwrap();
}
...
fn edge_detection(buffer:&Vec<RGB<u8>>, width:usize, height:usize, threshold:f32)->Vec<RGB<u8>>{
    ...
    for _row in 0..height{
        ...
        for _col in 0..width{
            //右下4个像素
            match((calc_pixel(buffer.get(i+1), !col_binary, &buffer[i], threshold)
                        + calc_pixel(buffer.get(i+2), col_binary, &buffer[i], threshold)
